...
const AdminLoginModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isSuperAdmin && !selectedEmpresaId) {
      setError(
        "Debes seleccionar una empresa para entrar como administrador de sede.",
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!allowedRoles.includes(data.user.rol)) {
        throw new Error(
          "No tienes permisos administrativos para acceder a esta zona.",
        );
      }
  ...
  return (
    ...
    >
      <div className="text-center mb-6">
        <div className="inline-flex p-3 bg-sky-500/10 rounded-full text-sky-500 mb-3">
          <Shield size={32} />
        </div>
        <p className="text-slate-500 text-sm">
          Zona restringida para personal autorizado.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <Shield
              size={18}
              className={isSuperAdmin ? "text-sky-500" : "text-slate-400"}
            />
            <div>
              ...
              <p className="text-[10px] text-slate-500">
                Acceso global sin empresa específica
              </p>
            </div>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            />
            <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-sky-300 dark:peer-focus:ring-sky-800 rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-slate-600 peer-checked:bg-sky-500"></div>
          </label>
        </div>
        ...
        {!isSuperAdmin && (
          <div className="animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="relative">
              <select
                className="w-full px-4 py-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-white focus:border-sky-500 focus:ring-1 focus:ring-sky-500 outline-none transition-all appearance-none cursor-pointer"
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 dark:text-white transition-all"
        ...
        <div className="pt-4">
          <button
            type="submit"
            disabled={loading}
            className="w-full px-6 py-3 bg-gradient-to-r from-sky-400 to-blue-600 hover:from-sky-500 hover:to-blue-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-lg shadow-sky-950/20 liquid-glass-btn"
          >
            {loading && <Loader2 size={18} className="animate-spin" />}
            {isSuperAdmin
              ? "Entrar al Panel Global"
              : "Entrar a Administrar Sede"}
          </button>
        </div>
      </form>
    </ModalForm>
