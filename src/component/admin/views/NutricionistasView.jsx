...
const NutricionistasView = ({ empresa, token }) => {
  ...
  return (
    <div className="bg-white dark:bg-[#0a1128] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 h-full">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={handleNewClick} className="btn-brand px-3 py-2 rounded-lg transition-colors shadow-md flex items-center gap-2 text-sm font-bold">
          <Plus size={16} /> Nuevo
        </button>
      </div>

      {loading ? (
        <div className="text-center py-10 text-sky-500"><Loader2 className="animate-spin inline" size={32} /></div>
      ) : nutris.length === 0 ? (
        <div className="text-center py-16 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50/50 dark:bg-slate-800/20">
          ...
          <button onClick={handleNewClick} className="text-sky-500 font-bold text-sm hover:underline">Crear el primero</button>
        </div>
      ) : (
          ...
          {nutris.map(n => (
            <div key={n.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors border border-transparent hover:border-slate-200 dark:hover:border-slate-700 group">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-sky-400 to-blue-600 text-white flex items-center justify-center font-bold shadow-md text-sm">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
                ...
                <input autoFocus={!editingNutri} type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} className="w-full px-4 py-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 dark:text-white transition-all" placeholder="Ej: Juan" required />
            </div>
            <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Apellido</label>
                <input type="text" value={apellido} onChange={(e) => setApellido(e.target.value)} className="w-full px-4 py-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 dark:text-white transition-all" placeholder="Ej: Pérez" required />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Usuario</label>
                <input type="text" value={usuario} onChange={(e) => setUsuario(e.target.value)} className="w-full px-4 py-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 dark:text-white transition-all disabled:opacity-50" placeholder="Ej: jperez" required />
            </div>
            <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Email</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 dark:text-white transition-all" placeholder="juan@mail.com" required />
            </div>
          </div>
          <div>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 dark:text-white transition-all" placeholder={editingNutri ? "Dejar en blanco para mantener la actual" : "••••••••"} required={!editingNutri} minLength={6}/>
          </div>
          <div className="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" disabled={saving} className="btn-brand px-6 py-2 rounded-lg text-sm flex items-center gap-2 shadow-lg transition-colors">
